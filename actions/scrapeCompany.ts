...
function logStep(status: string, details?: string) {
  ...
  const message = `${time} | [scrapeCompany] ${status}${details ? ": " + details : ""}`;
  console.log(message);
}
...
async function searchDuck(query: string): Promise<string | null> {
  logStep("Searching DuckDuckGo", query)
...
async function summarize(text: string, context: string): Promise<string> {
  ...
  logStep("Summary:", object.summary.slice(0, 60) + "…")
  return object.summary
}

export async function scrapeCompany(company: string) {
...
`;
  ...
  logStep("Synthesizing final CompanyData")
  ...
  logStep("Final object", JSON.stringify(result, null, 2))
  return result
}
